...
{
    /// <summary>
    /// Interaction logic for ResidentialSearch.xaml
    /// </summary>
    public partial class ResidentialSearch : Window
    {
        public OleDbConnection cidDB = MainWindow.cidDB;
        public static ListCollectionView AddressView { get; set; }
        public List<ResidentialLocation> list { get; set; }
        public Window ParentWindow { get; set; }

        public ResidentialSearch(Window parentwindow)
        {
            InitializeComponent();
            InitializeForm();
        ...
        private void GetAddresses()
        {
            ...
            string strSQL = "SELECT * FROM tbl_Complaint_Residential_Sites";

            OleDbCommand cidSQL = new OleDbCommand(strSQL, cidDB);
            OleDbDataReader cidReader = cidSQL.ExecuteReader();

            list = new List<ResidentialLocation>();
            while (cidReader.Read())
            {
                ResidentialLocation obj = new ResidentialLocation();
                ...
                //if (cidReader[12] != DBNull.Value) obj.BuildingDescription = cidReader.GetString(1);
        ...
        {
            ResidentialLocation comp = (ResidentialLocation)item;
            bool match = true;

            //if (txtDesc1.Text != "") match = (comp.BuildingDescription.IndexOf(txtDesc1.Text, StringComparison.CurrentCultureIgnoreCase) != -1);
        ...
        private void listAddresses_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            ...
            if (row.Item != null)
            {
                if (ParentWindow is ComplaintWizard) ((ComplaintWizard)ParentWindow).SetResidentialAddress(((ResidentialLocation)row.Item));
                else if (ParentWindow is ResidentialComplaintForm) ((ResidentialComplaintForm)ParentWindow).SetComplaintAddress(((ResidentialLocation)row.Item).ID);
                Close();
            }
